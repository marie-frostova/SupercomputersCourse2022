...
template<class T, class R, class P, class C, class W, class WF>
class ThreadPool {
private:
	P& producer;
	C& consumer;
	...
	void WorkerThread() {
		W worker = workerFactory.create();
		for (;;) {
			T task;
			if (!producer.produce(task))
				break;
			R result;
...
public:
	ThreadPool(P& producer0, C& consumer0, WF& workerFactory0)
		: producer(producer0),
		  consumer(consumer0),
...
public:
	...
	bool produce(RayTracingTask& task) {
...
class RayTracingResultConsumer {
private:
	Image &image;
...
public:
	...
	void consume(RayTracingResult& result) {
...
public:
	...
	void run(RayTracingTask& task, RayTracingResult& result) {
		result.task = task;
...
public:
	...
	RayTracingWorker create() {
		return RayTracingWorker(viewPlane, scene, numOfSamples);
	}	
};
...
#define THREAD_POOL
	...
	ThreadPool<
		RayTracingTask,
		RayTracingResult,
		RayTracingTaskProducer,
		RayTracingResultConsumer,
		RayTracingWorker,
		RayTracingWorkerFactory
	> threadPool(
		producer,
		consumer,
		workerFactory
